...
export const SPECS = {
  salary(x) {
    ...
    return {
      ...
      sheets: [{
        ...
        sections: [
          { label: 'Cost to company', rows: [['Basic + DA', r.basic, r.basic / 12], ['HRA', r.hra, r.hra / 12], r.conveyance ? ['Conveyance allowance', r.conveyance, r.conveyance / 12] : null, r.variable ? ['Variable pay', r.variable, r.variable / 12] : null, ['Special allowance (the balance)', r.special, r.special / 12], r.employerNps ? ['Employer NPS (80CCD(2))', r.employerNps, r.employerNps / 12] : null, r.employerPf ? ['Employer PF', r.employerPf, r.employerPf / 12] : null, r.gratuity ? ['Gratuity provision', r.gratuity, r.gratuity / 12] : null, bold(['Cost to company', r.ctc, r.ctc / 12])].filter(Boolean) },
  ...
  home(x) {
    ...
    const sheets = [
      { name: 'Cost', headline: `₹${fmtINR(c.total)} to actually own a ₹${fmtINR(c.price)} home in ${c.city}`, columns: [{ header: 'Item', width: 64 }, { header: 'Amount', width: 20, fmt: X.inr }, { header: '% of price', width: 12, fmt: X.pct }], rows: [...c.lines.map((l) => (l.kind === 'price' ? bold([l.label, l.amount, '']) : [l.label, l.amount, c.price ? l.amount / c.price : 0])), bold(['All-in cost', c.total, c.price ? c.total / c.price : 0]), ['Of which not on the sticker', c.hiddenTotal, c.hiddenPct], loan > 0 ? ['Your money in total (all-in cost less loan)', cashNeeded, ''] : null].filter(Boolean), note: [...c.warnings, ...c.notes].join(' ') },
    ];
    if (plan) {
      sheets.push({ name: 'Funding', headline: `Loan ₹${fmtINR(loan)}, EMI ₹${fmtINR(emi)} at ${(+st.ratePct).toFixed(2)}% for ${Math.round(tenureMonths / 12)} years`, columns: [{ header: 'Item', width: 52 }, { header: 'Value', width: 22, fmt: X.inr }], rows: [['Price', c.price], ['Down payment', downPayment], bold(['Loan', loan]), ['Loan as a share of price', `${(100 * loan / (c.price || 1)).toFixed(0)}%`], bold(['Monthly EMI', emi]), ['Cash at booking', plan.cashAtBooking], ['Pre-EMI interest before possession', plan.preEmiTotal], ['GST paid with the stages', plan.gstTotal], ['Paid by you by possession', plan.youByPossession], ['EMI starts in month', plan.emiStartMonth]] });
      sheets.push({ name: 'Payment plan', headline: plan.constructionMonths > 0 ? `Pre-EMI interest ₹${fmtINR(plan.preEmiTotal)} over ${plan.constructionMonths} months; EMI from month ${plan.emiStartMonth}` : 'One payment on registration; EMI from the following month', columns: [{ header: 'Month', width: 8, fmt: '0' }, { header: 'Stage', width: 32 }, { header: 'Builder demands', fmt: X.inr }, { header: 'From you', fmt: X.inr }, { header: 'Bank releases', fmt: X.inr }, { header: 'Loan released so far', fmt: X.inr }, { header: 'Pre-EMI until next stage', fmt: X.inr }], rows: plan.rows.map((row) => [row.month, `${row.label} (${+row.pct.toFixed(1)}%)`, row.due + row.gst, row.fromYou, row.fromBank, row.cumulativeLoan, row.preEmiUntilNext || 0]), note: plan.warnings.join(' ') });
    }
    return {
      title: 'TaxCompass India: buying a home',
      sheets,
      inputs: [['City', st.city], ['Agreed price', +st.price], ['What you are buying', st.status], ['Registered in the name of', st.buyer], ['Affordable housing (GST 1%)', st.affordable ? 'Yes' : 'No'], ['Brokerage (%)', +st.brokerageRate || 0, '0.00'], ['Interiors', +st.interiors || 0], ['Legal and valuation', +st.legalAndValuation || 0], plan ? ['Loan wanted', loan] : null, plan ? ['Down payment', downPayment] : null, plan ? ['Interest rate (%)', +st.ratePct, '0.00'] : null, plan ? ['Tenure (years)', Math.round(tenureMonths / 12), '0'] : null, plan ? ['Loan processing fee (%)', +st.processingFeeRate || 0, '0.00'] : null].filter(Boolean),
      notes: ['Stamp duty, registration and GST rates are from data/property_charges.json with a source, date and confidence per city; rates marked corroborated or unverified should be confirmed with the sub-registrar or lender.', 'The loan is the amount you said you would borrow; whether a lender sanctions it depends on their own policy. RBI caps a home loan at 90% of the price up to 30 lakh, 80% up to 75 lakh and 75% above; charges are never financed.', 'Your own money is used before any loan is released. Pre-EMI interest is charged monthly on the released amount at the loan rate; interest paid before possession is deductible in five equal yearly instalments from the year of possession, within the 2,00,000 self-occupied cap, old regime only.', 'If the price is 50 lakh or more, deduct 1% TDS from what you pay the seller (Form 26QB).'],
    };
  },
};
